...
-- One row per task: selected task attrs + processing metrics + is_auto + custom task fields
...
task_attrs AS (
  SELECT
    ...
    json_extract_scalar(t.custom_issue_type, '$[0]') AS issue_type,
    json_extract_scalar(t.custom_issue_category, '$[0]') AS issue_category,
    t.custom_app AS app,
    t.custom_resolution_type AS resolution_type,
    t.custom_user_type AS user_type,
