...
csat_score_events AS (
    SELECT
        b.ticket_id,
        b.created_at AS csat_score_created_at,
        b.events__id AS csat_event_id,
        b.events__value AS csat_val
    FROM base_audit b
    WHERE b.events__type = 'Change'
      AND b.events__field_name = 'satisfaction_score'
      AND b.events__value IN ('good', 'bad')
),
tech_team AS (
...
csat_ranked_prior AS (
    SELECT
        ...
        am.csat_author_id,
        ROW_NUMBER() OVER (
            PARTITION BY s.ticket_id, s.csat_event_id
            ORDER BY am.agent_message_created_at DESC, am.agent_message_event_id DESC
        ) AS rn_prior
    FROM csat_score_events s
    LEFT JOIN csat_agent_messages am
      ON am.ticket_id = s.ticket_id
     AND (
            am.agent_message_created_at < s.csat_score_created_at
            OR (
                am.agent_message_created_at = s.csat_score_created_at
                AND am.agent_message_event_id < s.csat_event_id
            )
        )
),
...
csat_best_prior AS (
    SELECT
        ...
        agent_message_event_id,
        csat_author_id
    FROM csat_ranked_prior
    WHERE rn_prior = 1
),
...
csat_attr AS (
    SELECT
        s.ticket_id,
        COALESCE(p.agent_message_created_at, a.agent_message_created_at) AS created_at,
        COALESCE(p.agent_message_event_id, a.agent_message_event_id) AS events__id,
        COALESCE(p.csat_author_id, a.csat_author_id) AS csat_author_id,
        s.csat_event_id,
        s.csat_score_created_at,
        s.csat_val,
        CASE
        ...
        END AS csat_match_rule
    FROM csat_score_events s
    LEFT JOIN csat_best_prior p
      ON p.ticket_id = s.ticket_id
     AND p.csat_event_id = s.csat_event_id
    LEFT JOIN csat_best_any a
      ON a.ticket_id = s.ticket_id
     AND a.csat_event_id = s.csat_event_id
),
...
FROM final_log fl
    ...
    LEFT JOIN csat_attr ca ON ca.ticket_id = fl.ticket_id
                         ...
                         AND (ca.csat_author_id IS NULL OR fl.author_id = ca.csat_author_id)
