...
-- stats по любым cancel-ивентам на юзера (как раньше)
cancel_stats_any AS (
  SELECT
    user_id,
...
-- 2) типизируем cancel -> upsell/main
...
-- 3) все подписки/платежи пользователей, которые жали cancel
...
-- 4) 1 строка на vendor_subscription_id
sub_instances AS (
  SELECT
    user_id,
...
-- 5) матчим cancel-ивенты на конкретные подписки (может быть много ивентов на одну подписку)
...
-- 6) агрегируем cancel-ивенты в 1 строку на ОТМЕНЁННУЮ подписку
...
-- A) cancel upsell -> rebill по этой же upsell
cand_upsell_cancel_rebill AS (
  SELECT
    ...
    MIN(ps.purchase_completed_at) AS rebill_purchase_completed_at,
...
-- B) cancel main -> rebill по этому же main
...
-- C) cancel main -> rebill по upsell-дочерним подпискам
...
-- 7) объединяем кандидатов и дедупим 1 строка на subscription_id
all_candidates AS (
...
SELECT
  ...
  d.rebill_purchase_completed_at,
