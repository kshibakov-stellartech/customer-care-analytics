...
/* =========================================================
tickets_attr temporarily commented out
========================================================= */
/*
tickets_attr AS (
    SELECT
           ticket_id,
           MAX(CASE WHEN events__field_name = 'assignee_id' AND events__value IS NOT NULL THEN events__value END) AS assigned_to
    FROM base_audit
    GROUP BY 1
),
*/
...
WHERE ad.agent_id <> 26440502459665
ORDER BY ticket_id, created_at, COALESCE(msg_created_at, assign_created_at)
)

SELECT CAST(created_at as DATE) as created_date,
       AVG(duration_sec_agent) as avg_reply_agent,
       AVG(duration_sec_overall) as avg_reply_overall,
       COUNT(*) as first_reply_cnt
FROM res
WHERE 1=1
  AND agent_reply_num = 1
  AND duration_sec_overall >= 600
GROUP BY 1
ORDER BY 1
;
