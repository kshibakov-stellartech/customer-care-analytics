...
SELECT ticket_id,
       created_at,
       events__id,
       LAG(created_at) OVER(PARTITION BY ticket_id ORDER BY created_at) as prev_created_at
FROM tickets
...
/*
SELECT *
FROM ticket_lost_time tlt
    LEFT JOIN data_bronze_zendesk_prod.zendesk_audit za ON tlt.ticket_id = za.ticket_id
                                                  AND za.events__type IN ('Comment', 'Notification')
                                                  AND tlt.prev_created_at <= za.created_at
                                                  AND tlt.created_at > za.created_at
...
SELECT ticket_id,
       ...
       events__value as status,
       events__previous_value as prev_status,
       LAG(created_at) OVER(PARTITION BY ticket_id ORDER BY created_at) as prev_status_created_at,
       DATE_DIFF('second', LAG(created_at) OVER(PARTITION BY ticket_id ORDER BY created_at), created_at) as status_proccessing_time
FROM tickets
    JOIN data_bronze_zendesk_prod.zendesk_audit za USING(ticket_id)
WHERE 1=1
  AND events__field_name = 'status'
),
    ticket_customer_duration AS (
SELECT ticket_id,
       SUM(CASE WHEN status = 'open' THEN status_proccessing_time ELSE 0 END) as customer_waiting_time
...
-------------------------------------------------
       ...
       COUNT(CASE WHEN events__field_name = 'assignee_id' AND events__value IS NOT NULL THEN events__value END) as assignees_count,
       ...
       --tech_team_time (asana task's data is needed for "completion metric")

       --avg_reply_time_total
...
-------------------------------------------------
       ...
       --avg_reply_time_agent
       --first_reply_time_agent
       --second_reply_time_agent
       --avg_concecutive_reply_time_agent

       --avg_reply_time_auto
       --first_reply_time_auto
       --second_reply_time_auto
       --avg_concecutive_reply_time_auto
