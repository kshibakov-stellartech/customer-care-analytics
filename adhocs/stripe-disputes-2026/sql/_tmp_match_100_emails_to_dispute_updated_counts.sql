...
dispute_updated_emails AS (
  SELECT DISTINCT lower(data__object__evidence__customer_email_address) AS user_email
  FROM data_bronze_stripe_prod.stripe_charge_dispute_updated
  WHERE data__object__evidence__customer_email_address IS NOT NULL
),
status AS (
  SELECT i.row_num, i.email, i.ticket_id,
         CASE WHEN d.user_email IS NOT NULL THEN 1 ELSE 0 END AS is_found
  FROM input_emails i
  LEFT JOIN dispute_updated_emails d
