...
matched AS (
    ...
    LEFT JOIN disputes d
      ON i.email = d.user_email
)
SELECT
  COUNT(*) AS input_rows,
  COUNT_IF(dispute_id IS NOT NULL) AS found_rows,
  COUNT_IF(dispute_id IS NULL) AS missing_rows,
  COUNT(DISTINCT CASE WHEN dispute_id IS NOT NULL THEN email END) AS found_unique_emails,
  COUNT(DISTINCT CASE WHEN dispute_id IS NULL THEN email END) AS missing_unique_emails
FROM matched;
