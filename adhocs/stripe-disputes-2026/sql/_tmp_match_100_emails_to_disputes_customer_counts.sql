...
dispute_emails AS (
  SELECT DISTINCT
    lower(cu.customer_email) AS user_email
  ...
  LEFT JOIN customer_latest cu
   ...
   AND cu.rn = 1
  WHERE cu.customer_email IS NOT NULL
),
status AS (
  SELECT i.row_num, i.email, i.ticket_id,
         CASE WHEN d.user_email IS NOT NULL THEN 1 ELSE 0 END AS is_found
  FROM input_emails i
  LEFT JOIN dispute_emails d ON i.email = d.user_email
)

SELECT COUNT(*) AS input_rows,
       SUM(is_found) AS found_rows,
       COUNT(*) - SUM(is_found) AS missing_rows
FROM status;
