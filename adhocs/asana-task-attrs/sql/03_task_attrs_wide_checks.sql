WITH final AS (
  -- paste query from 03, narrowed to key fields for checks
  WITH base_tasks AS (
    SELECT DISTINCT pt.project_id, p.name AS project_name, pt.task_id, pt._fivetran_synced AS project_task_synced_at, t.created_at
    ...
    SELECT ts.task_id, ts.section_id, ts._fivetran_synced AS section_assignment_synced_at,
    ...
    SELECT bt.project_id, bt.project_name, bt.project_task_synced_at, t.id AS task_id, t.created_at
    FROM base_tasks bt JOIN fivetran_asana.task t ON t.id = bt.task_id
    ...
    SELECT s.target_id AS task_id, s.id AS story_id, s.created_at AS event_at, s.text,
      CASE
        WHEN regexp_like(lower(s.text), 'moved this task from ".*" to ".*"') OR regexp_like(lower(s.text), 'changed section from .* to .*') THEN 'section'
        ...
        ELSE NULL END AS changed_field,
      CASE
        WHEN regexp_like(lower(s.text), 'moved this task from ".*" to ".*"') THEN regexp_extract(s.text, 'from "([^"]*)" to "', 1)
      ...
      CASE
        WHEN regexp_like(lower(s.text), 'moved this task from ".*" to ".*"') THEN regexp_extract(s.text, 'to "([^"]*)"', 1)
        WHEN regexp_like(lower(s.text), 'changed section from .* to .*') THEN regexp_extract(s.text, '(?i) to (.*)$', 1)
        WHEN regexp_like(lower(s.text), 'changed section to .*') THEN regexp_extract(s.text, '(?i)changed section to (.*)$', 1)
        WHEN regexp_like(lower(s.text), 'changed task progress from .* to .*') THEN regexp_extract(s.text, '(?i) to (.*)$', 1)
        WHEN regexp_like(lower(s.text), 'changed .* to .*') THEN regexp_extract(s.text, '(?i)changed .* to (.*)$', 1)
        WHEN regexp_like(lower(s.text), 'assigned to ') THEN regexp_extract(s.text, '(?i)assigned to (.*)$', 1)
      ...
      CASE WHEN regexp_like(lower(s.text), 'moved this task from ".*" to ".*"') THEN 1
           WHEN regexp_like(lower(s.text), 'changed section from .* to .*') THEN 2
           ELSE 3 END AS dedupe_priority
    FROM fivetran_asana.story s
    JOIN base_tasks bt ON bt.task_id = s.target_id
    WHERE trim(coalesce(s.text, '')) <> ''
  ), history_attrs AS (
    SELECT task_id, event_at, changed_field, previous_value, current_value
    FROM (
      SELECT hr.*, row_number() OVER (
        PARTITION BY hr.task_id, coalesce(hr.changed_field, ''), coalesce(hr.previous_value, ''), coalesce(hr.current_value, ''), date_trunc('second', hr.event_at)
    ...
    SELECT task_id, event_at, trim(coalesce(previous_value, '')) AS previous_status, trim(coalesce(current_value, '')) AS current_status,
    ...
    FROM history_attrs WHERE changed_field IN ('section', 'task_progress')
  ), status_events_dedup AS (
    SELECT task_id, event_at, previous_status, current_status, current_group, previous_group
    FROM (
      SELECT se.*, lag(se.previous_status) OVER (PARTITION BY se.task_id ORDER BY se.event_at) AS prev_prev_status,
    ...
    WHERE NOT (x.previous_status = x.prev_prev_status AND x.current_status = x.prev_curr_status AND x.prev_event_at IS NOT NULL AND date_diff('second', x.prev_event_at, x.event_at) BETWEEN 0 AND 10)
  ), group_events AS (
    SELECT se.task_id, se.event_at, se.current_status, se.current_group,
           lag(se.current_group) OVER (PARTITION BY se.task_id ORDER BY se.event_at) AS prev_group
    FROM status_events_dedup se
  ), group_switches AS (
    SELECT task_id, event_at, current_status, current_group, prev_group
    FROM group_events
    WHERE prev_group IS NULL OR current_group <> prev_group
  ), first_final_exact AS (
    SELECT x.task_id, x.started_at, gs.current_status AS end_status, gs.event_at AS ended_at
    FROM (
      SELECT bt.task_id, bt.created_at AS started_at, MIN(gs.event_at) AS ended_at
      FROM base_tasks bt
      JOIN group_switches gs ON gs.task_id = bt.task_id AND gs.current_group = 'final'
      GROUP BY bt.task_id, bt.created_at
    ) x
    JOIN group_switches gs ON gs.task_id = x.task_id AND gs.event_at = x.ended_at AND gs.current_group = 'final'
  ), reopen_starts AS (
    ...
    SELECT task_id, date_diff('minute', started_at, ended_at) AS resolution_time_min,
           row_number() OVER (PARTITION BY task_id, started_at, ended_at ORDER BY ended_at) AS rn
    FROM periods_union
  ...
  SELECT ta.task_id, pa.avg_resolution_time_min
  FROM task_attrs ta
  LEFT JOIN processing_avg pa ON pa.task_id = ta.task_id
)
SELECT
  COUNT(*) AS rows_total,
  COUNT(DISTINCT task_id) AS tasks_distinct,
  COUNT(*) - COUNT(DISTINCT task_id) AS duplicate_rows,
  SUM(CASE WHEN avg_resolution_time_min < 0 THEN 1 ELSE 0 END) AS negative_avg_cnt
FROM final;
