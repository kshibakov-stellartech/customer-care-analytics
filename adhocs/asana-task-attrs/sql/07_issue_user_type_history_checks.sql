...
SELECT
  ...
  SUM(CASE WHEN rn = 1 THEN 1 ELSE 0 END) AS rows_after_dedup,
  COUNT(DISTINCT task_id) FILTER (WHERE attribute_name = 'issue_type' AND rn = 1) AS issue_type_tasks,
  COUNT(DISTINCT task_id) FILTER (WHERE attribute_name = 'user_type' AND rn = 1) AS user_type_tasks
FROM hist
